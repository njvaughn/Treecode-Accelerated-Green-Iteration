...
def setUpMesh(onlyFillOne=False):
    '''
...
#                        2, 2, 2/3 ,2/3 ,2/3 ]
    ...
    return x,y,z,w,atoms
     
    
def testGreenIterationsGPU_rootfinding(x,y,z,w,atoms,restartFile=None):
    
    startTime = time.time()


    numberOfTargets = len(x)                # set N to be the number of gridpoints.  These will be all the targets
    
    Energies, Times = greenIterations_KohnSham_SCF_rootfinding(scfTolerance, energyTolerance, numberOfTargets, gradientFree, symmetricIteration, GPUpresent, treecode, treecodeOrder, theta, maxParNode, batchSize, 
...
#                                 onTheFlyRefinement=onTheFlyRefinement, vtkExport=vtkExport)
    ...
    myData = [domainSize,0,0,0,0,order,numberOfTargets/order**3,numberOfTargets,gradientFree,
              divideCriterion,divideParameter1,divideParameter2,divideParameter3,divideParameter4,
              gaussianAlpha,gaugeShift,smoothingEps,energyTolerance,
              subtractSingularity,
              Energies['orbitalEnergies']-Energies['gaugeShift'], Energies['totalBandEnergy'], Energies['totalKinetic'], Energies['totalEx'], Energies['totalEc'], 
              Energies['totalEhartree'], Energies['totalEnergy'],
...
#               tree.E, tree.
#               tree.E, Energies['orbitalEnergies'][0], abs(tree.E+1.1373748), abs(Energies['orbitalEnergies'][0]+0.378665)]
...
def greensIteration_FixedPoint(psiIn):
    ...
    global orbitals, oldOrbitals, residuals, eigenvalueHistory
...
#                             print('type: ', type(gradientFree))
    ...
    if Energies['orbitalEnergies'][m]>0.0:
        Energies['orbitalEnergies'][m] = tree.gaugeShift - 0.5
...
#     residualVector = -(psiIn - orbitals[:,m])
    ...
    print('Orbital %i error and eigenvalue residual:   %1.3e and %1.3e' %(m,Energies['orbitalEnergies'][m]-tree.referenceEigenvalues[m]-tree.gaugeShift, eigenvalueDiff))
    ...
    myData = [m, greenIterationsCount, residuals,
              Energies['orbitalEnergies']-tree.gaugeShift, eigenvalueDiff]
...
#                 alpha = gaussianAlpha
    ...
    #     return
        ...
        print('Orbital Energy Errors after initialization: ', Energies['orbitalEnergies']-referenceEigenvalues[:tree.nOrbitals]-tree.gaugeShift)
    ...
    #     print('Updated totalElectrostatic:            %.10f H, %.10e H' %(tree.totalElectrostatic, tree.totalElectrostatic-Eelectrostatic))
        ...
        if printInitialEnergies==True:
            ...
            myData = [0, 1, Energies['orbitalEnergies'], tree.totalBandEnergy, tree.totalKinetic, 
        ...
        for m in range(tree.nOrbitals):
            if Energies['orbitalEnergies'][m] > tree.gaugeShift:
                Energies['orbitalEnergies'][m] = tree.gaugeShift - 1.0
...
#                     solverOptions={'fatol':1e-6, 'tol_norm':clenshawCurtisNorm, 'jac_options':jacobianOptions, 'line_search':None, 'disp':True}
                    ...
                    if np.abs(tree.eigenvalueDiff) < tol/10:
                        ...
                        targets = tree.extractPhi(m)
                        psiOut = np.append(targets[:,3], Energies['orbitalEnergies'][m])
                        Done=True
                    else:
...
#         occupations = computeOccupations(Energies['orbitalEnergies'], tree.nElectrons, Temperature)
...
#         print('Computing Vhartree and updating Veff took:    %.4f seconds. ' %hartreeConvolutionTime)
        ...
        print('Orbital Energy Errors after Veff Update: ', Energies['orbitalEnergies']-referenceEigenvalues[:tree.nOrbitals]-tree.gaugeShift)
        
        for m in range(tree.nOrbitals):
            print('Orbital %i error: %1.3e' %(m, Energies['orbitalEnergies'][m]-referenceEigenvalues[m]-tree.gaugeShift))
...
#             tree.exportGridpoints(filename)
        ...
        if printEachIteration==True:
            ...
            myData = [SCFcount, densityResidual, Energies['orbitalEnergies'], tree.totalBandEnergy, tree.totalKinetic, 
                      tree.totalEx, tree.totalEc, tree.totalEhartree, tree.E]
