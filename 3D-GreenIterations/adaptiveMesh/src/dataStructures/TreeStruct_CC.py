...
#     @da.delayed
    ...
    def buildTree_FirstAndSecondKind(self,maxLevels, divideCriterion, divideParameter1, divideParameter2=0.0, divideParameter3=0.0, divideParameter4=0.0, initializationType='atomic',savedMesh='', restart=False, printNumberOfCells=False, printTreeProperties = True, onlyFillOne=False): # call the recursive divison on the root of the tree
...
#         self.maxDepthAchieved, self.minDepthAchieved, self.treeSize, self.maxDepthCounter = recursiveDivide(self, self.root, maxLevels, divideCriterion, divideParameter, levelCounter, maxDepthCounter, saveList, printNumberOfCells, maxDepthAchieved=0, minDepthAchieved=maxLevels )#.compute()
        ...
        ## Refine to min depth
#         self.maxDepthAchieved, self.minDepthAchieved, self.treeSize, self.maxDepthCounter = recursiveDivide(self, self.root, self.minDepth, divideCriterion, divideParameter, levelCounter, maxDepthCounter, saveList, printNumberOfCells, maxDepthAchieved=0, minDepthAchieved=maxLevels )#.compute()
#         print('Number of cells after uniform refine: ', self.maxDepthCounter)
...
#         print('Number of cells after uniform refine: ', self.maxDepthCounter)
        ...
        for _,cell in self.masterList:
            if cell.leaf==True:
#                 print('Refining cell %s' %cell.uniqueID)
                self.maxDepthAchieved, self.minDepthAchieved, self.treeSize, self.maxDepthCounter = recursiveDivide(self, cell, maxLevels, divideCriterion, divideParameter, levelCounter, maxDepthCounter, saveList, printNumberOfCells, maxDepthAchieved=0, minDepthAchieved=maxLevels )#.compute()
...
#             self.printWavefunctionNearEachAtom(m)
        ...
        self.maxDepthAtAtoms = self.maxDepthAchieved # since not doing any additional refinement at atom
