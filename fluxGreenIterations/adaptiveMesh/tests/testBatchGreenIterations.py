...
#                 element[1].gridpoints[i,j,k].setAnalyticPsi(2)
    ...
    testFunction = 'sum(psi_i**2)'
    
    myData = [domainSize,minDepth,maxDepth,tree.numberOfGridpoints,testFunction,divideTol,energyResidual,
              energyErrorGS,psiL2ErrorGS,psiLinfErrorGS,
              energyErrorFES,psiL2ErrorFES,psiLinfErrorFES]
 
#     myFile = open('/home/njvaughn/Greens-Functions-Iterative-Methods/3D-GreenIterations/adaptiveMesh/results/accuracyResults.csv', 'a')
    myFile = open('/home/njvaughn/accuracyResults.csv', 'a')
    with myFile:
